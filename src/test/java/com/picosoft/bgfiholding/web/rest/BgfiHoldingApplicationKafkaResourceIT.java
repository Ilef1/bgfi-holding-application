package com.picosoft.bgfiholding.web.rest;
...
import com.picosoft.bgfiholding.IntegrationTest;
import com.picosoft.bgfiholding.config.EmbeddedKafka;
import com.picosoft.bgfiholding.config.KafkaSseConsumer;
import com.picosoft.bgfiholding.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class BgfiHoldingApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bgfi-holding-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/bgfi-holding-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/bgfi-holding-application-kafka/unregister"));
                return;
            }
        }
